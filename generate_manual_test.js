 ...
 * Usage: node generate_manual_test.js > manual_test.html
 */
const fs = require('fs');
...
const implementations = implementationFiles.map((file, idx) => {
...
const html = `<!-- 
    ...
    Run: node generate_manual_test.js > manual_test.html
-->
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  <div class="app-container">
    ...
    <div id="top-center" class="grid-cell"><svg id="top-lines"></svg></div>
    <div id="top-right" class="grid-cell"></div>

    <div id="left-center" class="grid-cell"><svg id="left-lines"></svg></div>
    ...
    <div id="center-center" class="grid-cell">
      ...
      <div id="impl-selector" style="position:absolute; top:50px; right:10px; background:rgba(0,0,0,0.5); padding:5px; border-radius:3px; display:flex; align-items:center; font-family:sans-serif; font-size:12px;">
        ...
        <select id="implSelect" style="padding:2px 5px;"></select>
      </div>
    </div>
  ...
  <script>${dynamicLoader}
    // Support URL param (?impl=src/implementation_B.js) with dropdown fallback
    ...
    scriptTag.onerror = () => {
      document.body.innerHTML = '<h1 style="color:red; font-family:sans-serif; padding:20px;">Error: Could not load script: ' + implFile + '</h1>';
    };
